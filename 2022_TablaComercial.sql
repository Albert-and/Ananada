...
WHERE T1.series <> '76' AND t1.DocDate BETWEEN CONVERT(date,GETDATE()-35) AND CONVERT(date,GETDATE()-1)

--------------------------

SELECT isnull(T6.baseref,'') as 'No. OV',T0.cardcode,T0.cardname,T2.Slpname as 'Asesor',T0.Doctotal,T0.Docnum,
T1.seriesname,T3.[ItemCode], T3.[Dscription], T3.[Quantity],T3.[LineTotal],T0.DocDate, T5.ItmsGrpNam as 'Categoría',
...
WHERE T0.CANCELED = 'N' AND T1.SeriesName <> 'SI' AND T0.DocDate BETWEEN CONVERT(date,GETDATE()-35) AND CONVERT(date,GETDATE()-1) AND T1.SeriesName='FAC'--and T3.ItemCode not like 'F%'

UNION ALL

SELECT isnull(T6.baseref,'')as 'No. OV',T0.cardcode,T0.cardname,T2.Slpname as 'Asesor',T0.Doctotal,T0.Docnum,T1.seriesname,T3.[ItemCode], T3.[Dscription], T3.[Quantity]*-1, T3.[LineTotal], T0.DocDate,T5.ItmsGrpNam as 'Categoría',T4.U_Codigo as 'Segmento',
...
inner join OITB T5 ON T5.ItmsGrpCod=T4.ItmsGrpCod
left join DLN1 T6 ON t6.ObjType=t3.basetype and t6.DocEntry=t3.baseentry and t6.LineNum=t3.BaseLine
WHERE T0.CANCELED = 'N' AND T1.SeriesName <> 'SI' AND T0.DocDate BETWEEN CONVERT(date,GETDATE()-35) AND CONVERT(date,GETDATE()-1) AND T1.SeriesName='NC'--and T3.ItemCode not like 'F%'

------------------

select T0.DocNum as 'Documento', T0.DocDate as 'Fecha de creación',
T0.UpdateDate as 'Fecha de liberación',T0.U_EstatusOV,
(select top 1 Descr 
from UFD1 
where FieldID = 29 and fldvalue = T0.U_EstatusOV) as 'Estatus ANANDA'
from ORDR T0
where T0.U_EstatusOV is not null and T0.DocDate BETWEEN CONVERT(date,GETDATE()-45) AND CONVERT(date,GETDATE()-1)
